由spring整合springMVC和mybatis。springMVC处理浏览器发送的请求，调用业务逻辑层，根据需求调用持久层，进行持久化操作

# 一、Mybatis简介

[B站视频](https://www.bilibili.com/video/BV1VP4y1c7j7?spm_id_from=333.337.search-card.all.click)

## MyBatis历史
-    MyBatis最初是Apache的一个开源项目iBatis, 2010年6月这个项目由Apache Software Foundation迁移到了Google Code。随着开发团队转投Google Code旗下，iBatis3.x正式更名为MyBatis。代码于2013年11月迁移到Github
- iBatis一词来源于“internet”和“abatis”的组合，是一个基于Java的持久层框架。iBatis提供的持久层框架包括SQL Maps（数据库和java的映射，即MyBatis封装JDBC的过程）和Data Access Objects（DAO）
## MyBatis特性
1. MyBatis 是支持定制化 SQL、存储过程以及高级映射的优秀 的持久层框架
...
## 创建maven工程
- 打包方式：jar
  在`<version>`  标签之下添加 `<packaging>jar<packaging>`
- 引入依赖:  `MyBatis , junit,  mysql`
...
public class UserMapperTest {
    ...
    public void testInsertUser() throws IOException {
		...
		SqlSession sqlSession = sqlSessionFactory.openSession(true);//这里的ture是autoCommit
...
> ```
>
> 第二次出错是数据库表字段不匹配，原因是创建表时没有性别，删掉后即可
>
> 第三次出错是age=23太长，改成了2就通过了。
...
- MyBatis获取参数值的两种方式：${}和#{}  
- <font color="orange">${}的本质就是<font size=5px>字符串拼接</font>，#{}的本质就是<font size=5px>占位符赋值 </font></font>
...
//mybatis会架将这些参数放入map中，以@Param("username")为键，以String username为值
//相当于  @Param(value = "username")
```
...
<select id="getEmpAndDept" resultMap="empAndDeptResultMapOne"><!--ResultMap 是自定义映射属性-->
...
<generatorConfiguration>
    ...
    <context id="DB2Tables" targetRuntime="MyBatis3Simple">
        ...
        <javaModelGenerator targetPackage="com.atguigu.mybatis.pojo" targetProject=".\src\main\java">
            <property name="enableSubPackages" value="true" /><!--是否使用子包：开启后每个点都是目录-->
...
public void testPageHelper() throws IOException {
	...
	EmpMapper mapper = sqlSession.getMapper(EmpMapper.class);
	//访问第一页，每页四条数据
	Page<Object> page = PageHelper.startPage(1, 4);/////////只要这一行就行：在查询功能之前开启
	List<Emp> emps = mapper.selectByExample(null);
	//在查询到List集合后，打印分页数据
}
```
- 分页相关数据：（Page 对象）（包含pageNum, pageSize, startRow, endRow, total, pages）
...
- 在查询获取list集合之后，使用`PageInfo<T> pageInfo = new PageInfo<>(List<T> list, intnavigatePages)`获取分页相关数据
	...
	- navigatePages：导航分页的页码数
	  ...
	  > 导航分页：如果在第一页，就显示 1 2 3 4 5
	  >
	  > ​				  如果在第五页，就显示 3 4 5 6 7
```java
@Test
public void testPageHelper() throws IOException {
	InputStream is = Resources.getResourceAsStream("mybatis-config.xml");
