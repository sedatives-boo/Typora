...
>
> <img src="../Resources/sum-of-root-to-leaf-binary-numbers.png" alt="img" style="zoom:50%;" />
>
> ```
...
class Solution {
    public boolean makesquare(int[] matchsticks) {
        ...
        Arrays.sort(matchsticks);//本来是使用 Collections.reverseOrder()但是会报错
        int[] edges = new int[4];
...
class Solution {
    public int jobScheduling(int[] startTime, int[] endTime, int[] profit) {
        ...
        //按结束时间排序
        Arrays.sort(jobs,(a,b)->a[1]-b[1]);
        int[] dp = new int[n+1];//dp[i] 表示当前i份工作可以获得的最大报酬
        for(int i = 1;i<n+1;i++){
            //先通过二分查到到 第 k 个工作 
            //第k份工作满足 他的结束时间刚好小于 第i+1的开始时间
            int k = binarySearch(jobs,i-1,jobs[i-1][0]);
            //在 jobs 数组中，在0~i-1之间找 第一个小于 jobs[i-1][0]的结束时间
            dp[i] = Math.max(dp[i-1],dp[k]+jobs[i-1][2]);
        }
        return dp[n];
    }
...
### 10.26、[862. 和至少为 K 的最短子数组](https://leetcode.cn/problems/shortest-subarray-with-sum-at-least-k/)🔴
...
方法一：DP+前缀和，找presum[x2]-presum[x1]的最小x2-x1
...
- 比如当`preSum[x2] <= preSum[x1]`（其中`x1 < x2`）时，表明`x1`到`x2`之间的元素的和是负数或0，那么就是当`preSum[xn] - preSum[x1] >= K`则必然有`preSum[xn] - preSum[x2] >= K`，那么这个时候我们只计算`xn - x2`即可（`x1`到`x2`之间的元素可以全部跳过了）
- 另一个角度，当`preSum[x2] - preSum[x1] >= K`时，`x1`就可以跳过了，为什么呢？因为`x1`到`x2`已经满足了大于`K`，再继续从`x1`开始向后再早，也不会再有比`x2`距离`x1`更近的了，毕竟我们要求的是最小的`x2 - x1`
...
### 10.27/907[子数组的最小值之和🔴](https://leetcode.cn/problems/sum-of-subarray-minimums/)
...
```java
//使用单调栈，对暴力的优化,通过左右数组记录每一个元素能影响到的左右边界。
class Solution {
...
>
> <img src="../Resources/17_telephone_keypad.png" alt="img" style="zoom: 50%;" />
>
> **示例 1:**
...
class Solution {
    ...
    for(String word:words){
        ...
        if(sb.toString().equals(num)) res.add(word);
    }
        return res;
    }
...
<img src="../Resources/f9fe31e0528ff89e6786f5c12f28fb12.png" alt="image-20220803200010103" style="zoom: 50%;" />
...
<img src="../Resources/696c8415e38eaea8299fb6ef05b62dc0.png" alt="image-20220803200121842" style="zoom:67%;" />
...
public class Solution {
    ...
    public int[] findBuilding (int[] heights) {
        ...
        Arrays.fill(ans, 1);
        for (int i = 0; i < n; i++) {
            int left = i-1, right = i+1;//从这栋楼的左右两边
            int leftmax = 0, rightmax = 0;
            while(left >= 0) {
                if (heights[left] > leftmax) {//只能一个比一个大
...
### 11/7 [816. 模糊坐标](https://leetcode.cn/problems/ambiguous-coordinates/)🔁
...
```java
//思路：1、先把字符串的括号去掉，只剩数字部分；2、枚举坐标中逗号的位置，把字符串分成两部分，这两部分可以分别考虑，且考虑的方法是一样的；3、对于分好的字符串，1）要么它自己代表一个整数：此时这个数字在位数大于1的时候，首位不能是0；2）要么它代表一个小数：枚举小数点的位置，整数部分的规则跟“1)”相同，小数部分的最后一位不能是0
class Solution {
    public List<String> ambiguousCoordinates(String s) {
        ...
        for(int i = 1;i<n-2;i++){
            List<String> part1 = fun(s.substring(0,i));
            List<String> part2 = fun(s.substring(i));
            for(String a :part1){
                for(String b:part2){
                    ans.add("("+a+", "+b+")");
                }
...
### 11.8、[764. 最大加号标志](https://leetcode.cn/problems/largest-plus-sign/)
...
>  **示例 1：**
>
> <img src="../Resources/plus1-grid.jpg" alt="img" style="zoom:50%;" />
>
> ```
> 输入: n = 5, mines = [[4, 2]]
> 输出: 2
...
### [(字典树 )剑指 Offer II 067. 最大的异或](https://leetcode.cn/problems/ms70jA/)
