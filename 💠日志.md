...
public class DiscussPost{
    ...
    //两个分词器，存储时采用尽可能多的分词
    @Field(type=FieldType.Text,analyzer="ik_max_word",searchAnalyzer = "ik_smart_")//文本类型，存储时解析器和搜索时解析器
...
在service包下创建elasticsearch包，创建名为 `DiscussPostRepository`类，加上@Repository
